...
const Calculator = {
  ...
  // this will hold the operator, we set it to null for new
  operator: null,
}
...
function Input_Digit(digit) {
  ...
  // we are checking to see if Wait_Second_Operand is truee and set
...
function Input_Decimal(dot) {
  ...
  if (!Calculator.Display_Value.includes(dot)) {
    // wa are saying that if the Display_Value does not contain a decimal point
...
function Handle_Operator(Next_Operator) {
  ...
  const Value_of_Input = parseFloat(Calculator.Display_Value);
  ...
  } else if (Calculator.operator) { // checks if an operator already exists
    const Value_Now = Calculator.First_Operand || 0;
    // if operator already exists, property lookup is performed for the operator
    // in the Perform_Calculation o\bject and the function that matches the
    // operator is executed
    const result = Perform_Calculation[Calculator.operator](Value_Now, Value_of_Input);
